...
def general_generation(generate_order, model_type, img_name, model, clip, processor, image_instance, token_mask, prompt, logger,
                          max_len=15, span_len=None, top_k=100, temperature=None,
    ...
    for iter_num in range(max_iters):
        if generate_order != "span":
            for ii in index_list:
                ...
                final_score = probs
                best_id = final_score.argmax(dim=1).view(-1, 1)
                inp[:, seed_len + ii] = idxs_.gather(1, best_id).squeeze(-1)
            ...
            for kk in range(max_len):
                ...
                for jj in range(batch_size):
                    ...
                    #                                                                        batch_text_list)
                    final_score = probs[jj, :]
                    best_id = final_score.argmax()
                    inp[jj][seed_len + kk] = idxs[jj][best_id]
    ...
    return gen_texts_list


def generate_caption_no_clip(img_name, model_type, model, clip, tokenizer, image_instance, token_mask, logger,
    ...
    generate_texts = general_generation(generate_order, model_type, img_name, model, clip, tokenizer, image_instance,
